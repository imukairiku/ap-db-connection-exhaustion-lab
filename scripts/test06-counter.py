...
if action=='gate':
 if state.get('stopped') or state['consecutive_failures']>=3: raise SystemExit('TEST-06 blocked after three consecutive failures')
 ...
 with open(tmp,'w') as f: json.dump(state,f,indent=2); f.write('\n'); f.flush(); os.fsync(f.fileno())

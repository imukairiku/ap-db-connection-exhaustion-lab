...
state = json.load(open(path, encoding="utf-8")) if os.path.exists(path) else {
    "schema_version": 1, "consecutive_failures": 0, "stopped": False, "history": []}
if action == "gate":
    if state.get("stopped") or state["consecutive_failures"] >= 3:
        raise SystemExit("TEST-03 blocked after three consecutive failures")
    ...
    state["history"] = (state["history"] + [{
        "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(), "run_id": run_id,
        "artifact_path": artifact, "status": status, "reason": reason}])[-3:]

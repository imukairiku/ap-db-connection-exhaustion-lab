...
PostgreSQLは`superuser_reserved_connections=3`のため、非superuserのAP枠は17本である。
旧AP 10本に対して新APは7本が接続でき、残り5本はPostgreSQLの実エラーで失敗する。
管理用superuser接続2本を保持し、観測query自身の1本を加えた時点で
`numbackends=20`を取得する。人工的な接続エラーは生成しない。

PASS条件は、旧AP残留10、新AP接続7以上、新AP接続失敗1以上、PostgreSQL
`max_connections=20`、観測時`numbackends=20`、旧APと新APの実backend識別、postmaster起動時刻と
